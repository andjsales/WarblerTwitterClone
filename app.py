...
def users_show(user_id):
    ...
    if g.user:
        likes_count = Likes.query.filter_by(user_id=g.user.id).count()
    else:
        likes_count = 0
...
def user_likes(user_id):
    user = User.query.get_or_404(user_id)
    liked_messages = Message.query.join(Likes, Message.id == Likes.message_id)\
...
def homepage():
    ...
    """
    if g.user:
        likes = [like.message_id for like in Likes.query.filter_by(
